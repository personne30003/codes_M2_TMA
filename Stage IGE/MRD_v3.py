...
Auteur : Evan LEMUR
MAJ 19 Mai 2024 :
simplification des fonctions MRD_segments et MRD : les résultats sont regroupés dans un seul dictionnaire (y
compris les spectres). Dommage, cette modification n'est pas rétrocompatible ...
...
def add_segment(dic,dic_segment,ignore='temps'):
    ...
    for key in dic.keys():
        ...
        if key==ignore:
            ...
            #new_dic[key]=np.concatenate([dic[key],dic_segment[key]],axis=0)
...
def MRD(x,y,name=('x','y'),get_spectres=True,filtrage=False,get_timescale=True,f_ech=1.0):
    ...
    if get_spectres==True:
        res[name[0]]=spectre_x
        res[name[0]+'_cum']=spectre1_cumul
        res[name[1]]=spectre_y
        res[name[1]+'_cum']=spectre2_cumul
    return res


def MRD_segments(x, y,name=('x','y'),filtrage=False,f_ech=1.0,get_spectres=True,bar_progress=False):
    ...
    if get_spectres==True:
        ...
        res[name[1]]=[]
        res[name[1]+'_cum']=[]
    iterateur_segments=tqdm(range(0,N_segments)) if bar_progress==True else range(0,N_segments)
...
def spectre_median(res_MRD):
    ...
    return new_res
def plot_spectre_moy(res_MRD,var,ax=None,**kwargs):
...
def gap_spectral_algo(spectre,spectre_sum,index,seuil=0.01,filtre=False):
    ...
    if filtre==True:
        spectre_copy=filtre_121(spectre_copy)
        spectre_sum_copy=np.cumsum(spectre_copy)
    signe_pic_spectre=signe_pic(spectre_copy)
    arg_pic=np.argmin(spectre_copy) if signe_pic_spectre==-1 else np.argmax(spectre_copy)
    pic=np.around(index[arg_pic],0)
    gap=0.0
    #iterateur=range(arg_pic,len(spectre_copy))
    for i in range(1,len(spectre_copy)):
        ...
        if (delta_spectre_sum/spectre_sum[i] <= seuil):
            gap=index[arg_pic]
            return pic,np.around(gap,0)
    return pic,np.NaN
